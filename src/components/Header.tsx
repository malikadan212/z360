...
export function Header({ phase, onNewCase }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white border-b border-slate-200">
      <div className="flex items-center justify-between h-14 px-4 sm:px-6 lg:px-8 max-w-screen-2xl mx-auto">

        {/* Logo */}
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-8 h-8 rounded-md gradient-bg flex items-center justify-center shrink-0">
            <FileCheck2 className="w-4 h-4 text-white" strokeWidth={2.5} />
          </div>
          <span className="font-semibold text-slate-900 text-base tracking-tight">Zikra</span>
          <span className="hidden md:block text-slate-300 select-none">·</span>
          <span className="hidden md:block text-slate-400 text-sm">Filing Readiness Copilot</span>
        </div>
